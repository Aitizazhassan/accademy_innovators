        ...
        <div class="content content-full content-boxed">
            ...
            <div class="block block-bordered block-rounded">
                <ul class="nav nav-tabs nav-tabs-alt" role="tablist">
                    <li class="nav-item">
                        <button class="nav-link space-x-1 {{ $errors->has('password') || $errors->has('current_password') ? '' : 'active' }}"
                            id="account-profile-tab" data-bs-toggle="tab" data-bs-target="#account-profile" role="tab"
                            aria-controls="account-profile"
                            aria-selected="{{ !$errors->has('password') || !$errors->has('current_password') ? 'true' : 'false' }}">
                ...
                <div class="block-content tab-content">
                    <div class="tab-pane {{ $errors->has('password') || $errors->has('current_password') ? '' : 'active' }}" id="account-profile"
                        role="tabpanel" aria-labelledby="account-profile-tab" tabindex="0">
                        @include('profile.partials.update-profile-information-form')
                    </div>
                    <div class="tab-pane {{ $errors->has('password') || $errors->has('current_password') ? 'active' : '' }}" id="account-password"
